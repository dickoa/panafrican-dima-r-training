...
* Assignment operation with <-
  + = also can be used but <- is conventional.
  
* Access to element with brackets, e.g., `vec_b[1]`


Take a look at the provided script for more examples, 1_pre_script_basic.R
...
?`readxl-package`
```
To see help of specific function, type the question mark with the package name, e.g., `?read_xls`.
```{r, eval = FALSE, echo = TRUE}
...
```
You can check which arguments are needed. Then type the argument as you need.
...
Take a look at the provided script, 2-3_pre_script_package_import.R
---
# Data import and export
